...
get_GOterm_semantic_mds <- function(goterm_enrich_file) {
  ...
  # load GO annotation for the whole proteome
...
cluster4_mds_plot <- cluster4_mds %>%
  ...
  geom_label(data = . %>%
               filter(GO %in% c("GO:0006486", "GO:0030203")) %>%
               summarise(D1 = mean(D1), D2 = mean(D2), .groups = "drop"),
             label = "Protein\nglycosylation", fill = alpha("white", 0.8), size = 3,
             label.padding = unit(0.3, "lines")) +

  geom_label(data = . %>%
               filter(GO %in% c("GO:0007015", "GO:0110053")) %>%
               summarise(D1 = mean(D1), D2 = mean(D2), .groups = "drop"),
             label = "Actin and cytoskeleton\norganization", fill = alpha("white", 0.8), size = 3,
             label.padding = unit(0.3, "lines")) +

  geom_label(data = . %>%
               filter(GO %in% c("GO:0035725", "GO:1903530", "GO:0006886")) %>%
               summarise(D1 = mean(D1), D2 = mean(D2), .groups = "drop"),
             label = "Vesicle-mediated and\nmacromolecule transport", fill = alpha("white", 0.8), size = 3,
             label.padding = unit(0.3, "lines")) +

  geom_label(data = . %>%
               filter(GO %in% c("GO:0006099", "GO:0043094", "GO:0006011")) %>%
               summarise(D1 = mean(D1), D2 = mean(D2), .groups = "drop"),
