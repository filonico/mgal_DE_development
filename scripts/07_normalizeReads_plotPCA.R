...
normalizeReads_plotPCA <- function(raw_counts_file) {
  ...
  return(list(rld_table = rld_table, vst_table = vst_table, panel = panel))
}

write("", stdout())
write("--- Analyzing gene raw counts ---", stdout())
gene_count_results <- normalizeReads_plotPCA("03_mapped_reads/gene_count_matrix.csv")

write("    Saving normalized read counts to files...", stdout())
write.table(gene_count_results$rld_table, file = "03_mapped_reads/gene_count_rlogTransformed.tsv", sep = "\t", quote = FALSE)
write.table(gene_count_results$vst_table, file = "03_mapped_reads/gene_count_vstTransformed.tsv", sep = "\t", quote = FALSE)

write("    Saving plot to file...", stdout())
ggsave("03_mapped_reads/PCA_generawcounts.pdf", plot = gene_count_results$panel, device = "pdf",
...
transcript_count_results <- normalizeReads_plotPCA("03_mapped_reads/transcript_count_matrix.csv")

write("    Saving normalized read counts to files...", stdout())
write.table(transcript_count_results$rld_table, file = "03_mapped_reads/transcript_count_rlogTransformed.tsv", sep = "\t", quote = FALSE)
write.table(transcript_count_results$vst_table, file = "03_mapped_reads/transcript_count_vstTransformed.tsv", sep = "\t", quote = FALSE)

write("    Saving plot to file...", stdout())
ggsave("03_mapped_reads/PCA_transcriptrawcounts.pdf", plot = transcript_count_results$panel, device = "pdf",
       dpi = 300, height = 6, width = 5.5, units = ("in"))
