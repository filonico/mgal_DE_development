...
normalizeReads_plotPCA <- function(raw_counts_file, runs_metadata_file) {
  ...
  # transform normalized read counts
  write("     Transforming read count with rlog...", stdout())
  suppressMessages(rld <- DESeq2::rlog(DESeq.ds, blind = FALSE))
  rld_table <- SummarizedExperiment::assay(rld)
  
  # transform normalized read counts
  write("     Transforming read count with vst...", stdout())
  suppressMessages(vst <- DESeq2::vst(DESeq.ds, blind = FALSE))
...
gene_counts_results <- normalizeReads_plotPCA("03a_mapped_reads_STAR/transcript_count_matrix.csv", "00_input/SRA_metadata.tsv")
...
write.table(gene_counts_results$vst_table, file = "04_PCA_readcounts/gene_counts_vstTransformed_norm.tsv", sep = "\t", quote = FALSE)

write("     Saving plot to file...", stdout())
ggsave("04_PCA_readcounts/PCA_generawcounts.pdf", plot = gene_counts_results$panel, device = "pdf",
       dpi = 300, height = 6, width = 5.5, units = ("in"), bg = 'transparent')

ggsave("04_PCA_readcounts/PCA_generawcounts.png", plot = gene_counts_results$panel, device = "png",
       dpi = 300, height = 6, width = 5.5, units = ("in"), bg = 'transparent')
